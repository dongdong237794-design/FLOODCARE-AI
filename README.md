...
>
> ### 🆕 อัปเดตล่าสุด (v2.6) — แก้ LIFF ส่งข้อมูลไม่ผ่าน (Unauthorized), เพิ่ม FAQ ค้นหาข้อมูลจริง, แก้ Render หลับ
>
> **1. แก้ "ส่งข้อมูลไม่สำเร็จ: Unauthorized" ตอนกดส่งฟอร์ม LIFF**
> - ต้นเหตุ: ตอนยืนยัน id token กับ LINE ต้องส่ง `client_id` เป็น **Channel ID** (ตัวเลขล้วน) แต่โค้ดเดิมส่ง **LIFF ID เต็ม** (เช่น `1234567890-AbCdEfGh`) ไป LINE จึงตอบกลับว่า token ไม่ถูกต้องเสมอ
> - แก้โดยตัดเอาเฉพาะส่วน Channel ID จาก LIFF ID ก่อนส่งยืนยัน (`split("-")[0]`)
> - เพิ่มระบบ fallback: ถ้า token ตรวจสอบไม่ผ่าน (เช่น เปิด LIFF ในเบราว์เซอร์นอก LINE หรือ token หมดอายุ) แต่มี `user_id` จาก `liff.getProfile()` แนบมาด้วย จะยอมให้บันทึกข้อมูลผ่านได้พร้อม log เตือนไว้ตรวจสอบทีหลัง ไม่บล็อกผู้ประสบภัยที่กำลังต้องการความช่วยเหลือ
>
> **2. เพิ่มฟีเจอร์ FAQ / ถามคำถามทั่วไปแบบค้นหาข้อมูลจริง**
> - เปิดใช้ Google Search Grounding ของ Gemini — AI ค้นหาข้อมูลปัจจุบันจริงจาก Google โดยอัตโนมัติ (เช่น ข่าวน้ำท่วมล่าสุด สถานการณ์น้ำวันนี้) แล้วเรียบเรียงเป็นภาษาไทยอ่านง่าย พร้อมแนบลิงก์แหล่งที่มาจริงท้ายข้อความเสมอ
> - เพิ่ม intent `FAQ` จับคำถามประเภท "ข่าวน้ำท่วมล่าสุด", "สถานการณ์น้ำ" ฯลฯ ให้ใช้โหมดค้นหานี้โดยตรง
> - ถ้าการค้นหาใช้ไม่ได้ (เช่น โควต้าหมด) จะ fallback กลับไปตอบแบบ AI ปกติอัตโนมัติ ไม่ error
>
> **3. แก้ปัญหา Render (free tier) sleep แล้วต้องรอ cold start ทุกครั้ง**
> - เพิ่ม endpoint `/health` สำหรับใช้กับบริการ keep-alive ฟรีอย่าง UptimeRobot (ดูวิธีตั้งค่าด้านล่าง)
>
> ### 🆕 อัปเดตล่าสุด (v2.7) — ย้าย SDK: `google-generativeai` (เลิกซัพพอร์ตแล้ว) → `google-genai`
>
> Google ประกาศเลิกซัพพอร์ตแพ็กเกจ `google-generativeai` (SDK เก่า) แล้ว ไม่มีอัปเดต/แก้บั๊กอีกต่อไป Render log จะมีคำเตือนนี้ขึ้นทุกครั้งที่ deploy:
> ```
> All support for the `google.generativeai` package has ended...
> Please switch to the `google.genai` package as soon as possible.
> ```
> **แก้แล้ว** โดยย้ายมาใช้ SDK ใหม่ `google-genai` (import path `from google import genai`) ทั้งหมด:
> - `requirements.txt`: เปลี่ยน `google-generativeai>=0.5.0` → `google-genai>=1.0.0`
> - `bot_config.py`: เปลี่ยนจากการสร้าง `genai.GenerativeModel(...)` แบบเก่า เป็นสร้าง `genai.Client(api_key=...)` แบบใหม่ (รูปแบบ client-based API) และเรียกผ่าน `client.models.generate_content(model=..., contents=..., config=...)` แทน
> - ระบบ Google Search Grounding (ฟีเจอร์ FAQ ในข้อ v2.6) เปลี่ยนมาใช้ `types.Tool(google_search=types.GoogleSearch())` ตามรูปแบบ SDK ใหม่
> - **ทดสอบแล้ว**: import โปรเจ็กต์ทั้งหมดผ่าน, เรียก `ask_gemini()` และ `ask_gemini_with_search()` (รวมการดึงลิงก์แหล่งอ้างอิงจาก grounding metadata) ผ่านทุกเทส, ทุก route (`/liff/sos`, `/liff/need`, `/liff/register`, `/health`) ตอบ 200 ปกติ
> - **ไม่กระทบฟีเจอร์อื่นใดๆ**: เปลี่ยนเฉพาะวิธีเรียก Gemini API ภายใน `bot_config.py` เท่านั้น ไม่มีการเปลี่ยน logic, intent, หรือ route อื่น
...
```bash
pip install flask line-bot-sdk google-genai gspread google-auth requests python-dotenv
```
...
### สรุป: ค่าอะไรบ้างที่ต้องใส่ใน Render?

ไปที่ Render > เลือก service ของบอท > แท็บ **Environment** > **Add Environment Variable** แล้วใส่ทีละตัวตามตารางนี้
(คีย์ในคอลัมน์ซ้ายคือชื่อตัวแปรที่ต้องพิมพ์ใน Render เป๊ะๆ ส่วนคอลัมน์ขวาคือค่าที่ต้องไปหามาใส่)

| ตัวแปร (Key) | ต้องใส่ไหม | ได้ค่ามาจากไหน |
|---|---|---|
| `LINE_CHANNEL_ACCESS_TOKEN` | ✅ จำเป็น | LINE Developers Console > Messaging API > Channel access token |
| `LINE_CHANNEL_SECRET` | ✅ จำเป็น | LINE Developers Console > Basic settings > Channel secret |
| `GEMINI_API_KEY` | ✅ จำเป็น | [Google AI Studio](https://aistudio.google.com/apikey) > Create API key |
| `GOOGLE_SHEET_ID` | ✅ จำเป็น | จาก URL ของ Google Sheet เช่น `docs.google.com/spreadsheets/d/{ตรงนี้}/edit` |
| `GOOGLE_SERVICE_ACCOUNT_JSON` | ✅ จำเป็น | ดูวิธีด้านบน (ทั้งไฟล์ JSON วางเป็นค่าเดียว) |
| `SOS_LIFF_ID`, `SOS_LIFF_URL` | ✅ จำเป็น (ถ้าจะใช้ฟอร์ม SOS) | สร้างจาก LINE Developers Console ตาม [หัวข้อ 5.1](#liff-setup) |
| `NEED_LIFF_ID`, `NEED_LIFF_URL` | ✅ จำเป็น (ถ้าจะใช้ฟอร์มขอของ) | สร้างตาม [หัวข้อ 5.2](#liff-setup) |
| `REGISTER_LIFF_ID`, `REGISTER_LIFF_URL` | ✅ จำเป็น (ถ้าจะใช้ฟอร์มลงทะเบียน) | สร้างตาม [หัวข้อ 5.3](#liff-setup) |
| `DASHBOARD_PASSWORD` | แนะนำ | ตั้งรหัสผ่านเอง (ใช้ดูหน้า `/dashboard`) |
| `FLASK_SECRET_KEY` | แนะนำ | สร้างด้วย `python3 -c "import secrets; print(secrets.token_hex(32))"` |
| `TMD_ACCESS_TOKEN` | แนะนำ (ถ้าจะให้เช็คสภาพอากาศได้) | สมัครที่ [data.tmd.go.th](https://data.tmd.go.th) (กรมอุตุนิยมวิทยา) |
| `WATER_DATA_MAX_AGE_MINUTES`, `CACHE_TTL_SECONDS`, `RATE_LIMIT_REQUESTS`, `RATE_LIMIT_WINDOW`, `SESSION_TTL_MINUTES` | ไม่บังคับ | ปรับแต่งประสิทธิภาพ ใช้ค่า default ได้เลยถ้าไม่แน่ใจ (ดูค่า default ใน `.env.example`) |

ใส่ครบแล้วกด **Save Changes** ที่มุมขวาล่าง Render จะ deploy ใหม่ให้อัตโนมัติ (ใช้เวลา ~2-3 นาที)
...
## 5.6 ป้องกัน Render หลับ (Keep-Alive ด้วย UptimeRobot — ฟรี 100%) <a name="keep-alive"></a>

**ตอบคำถามที่พบบ่อย: UptimeRobot ฟรีจริงไหม?** ใช่ครับ ฟรี 100% ไม่ต้องผูกบัตรเครดิต แพลนฟรีของ
UptimeRobot รองรับ monitor ได้ 50 ตัว เช็คทุก 5 นาที ซึ่งเกินพอสำหรับใช้ปลุก Render 1 service

**ทำไมต้องทำ:** Render แพลน Free จะ "หลับ" (sleep) อัตโนมัติหลังไม่มีคนเข้าใช้งาน 15 นาที พอมีคนทักบอท
ครั้งแรกหลังจากนั้น ต้องรอ 30-60 วินาทีให้ระบบตื่นก่อนถึงจะตอบกลับได้ (cold start) — การ ping `/health`
ทุก 10 นาทีจะทำให้ Render ไม่มีโอกาสหลับเลย

**ขั้นตอนตั้งค่า (ใช้เวลา ~3 นาที):**

1. เปิด [https://uptimerobot.com](https://uptimerobot.com) → กด **Sign Up** (สมัครฟรีด้วยอีเมล ไม่ต้องใส่บัตร)
2. ยืนยันอีเมลแล้ว login เข้าหน้า Dashboard
3. กดปุ่ม **"+ Add New Monitor"** สีเขียวมุมขวาบน
4. ตั้งค่าตามนี้:
   - **Monitor Type**: เลือก `HTTP(s)`
   - **Friendly Name**: ตั้งชื่อเล่นอะไรก็ได้ เช่น `FLOODCARE AI Keep-Alive`
   - **URL**: ใส่ `https://your-app.onrender.com/health` (แทน `your-app` ด้วยชื่อ service จริงของคุณบน Render)
   - **Monitoring Interval**: เลือก `10 minutes` (แพลนฟรีเลือกได้ต่ำสุด 5 นาที แต่ 10 นาทีพอแล้วและประหยัด quota)
5. กด **"Create Monitor"**
6. เสร็จแล้ว — UptimeRobot จะเริ่ม ping `/health` ทุก 10 นาทีทันที ไม่ต้องทำอะไรเพิ่ม

**ผลพลอยได้ที่ได้ฟรีไปด้วย:** ถ้า Render ล่มจริงๆ (ไม่ใช่แค่หลับ) UptimeRobot จะส่งอีเมลแจ้งเตือนให้ทันที
และมีหน้า status page (เช่น `https://stats.uptimerobot.com/xxxxx`) ให้ดูสถิติ uptime ย้อนหลังได้ฟรีด้วย

> ℹ️ Endpoint `/health` ถูกเพิ่มไว้ในโค้ดให้แล้วตั้งแต่ v2.6 — ไม่ต้องแก้โค้ดเพิ่ม แค่ตั้งค่าบน UptimeRobot ตามขั้นตอนข้างต้นเท่านั้น
> ⚠️ ถ้าอยากให้ไม่มีดีเลย์เลยแม้แต่ครั้งแรก (รับประกัน 100%) ต้องอัปเกรด Render เป็นแพลน `Starter` ($7/เดือน)
> ซึ่งไม่ sleep อยู่แล้วโดยไม่ต้องพึ่ง UptimeRobot — แต่ถ้างบจำกัด วิธี UptimeRobot ด้านบนนี้ฟรีและได้ผลดีในเกือบทุกกรณี
