...
from bot_config import (
    ...
    # Services
    ask_gemini, get_live_weather_scraper, sheets_mgr,
    ...
    WATER_LEVEL_SOURCE_URL, SNAKE_BITE_HOTLINE, SNAKE_BITE_INFO_URL,
    LINE_CHANNEL_SECRET,
...
def handle_follow(event):
    ...
    try:
        if REGISTER_LIFF_URL:
            ...
            # Register LIFF not configured yet -> fall back to chat-based registration
            # (a reply token can only be used once, so _start_registration sends the
            # single reply here instead of greeting separately)
...
def _start_sos_flow(event, user_id):
    """Start SOS workflow - redirect to LIFF immediately, NO registration gate.

    ⚠️ ในสถานการณ์ฉุกเฉิน ผู้ใช้ต้องแจ้งเหตุได้ทันที
    ข้อมูลการลงทะเบียนเก็บไว้กรอกในแบบฟอร์ม LIFF หรือหลังจากแจ้งเหตุแล้ว
    """
    session = sessions.get(user_id)
    session.update(state="sos_location")
    update_legacy_state(user_id, "sos_location", {})

    # Open LIFF for full SOS experience
    if SOS_LIFF_URL:
        line_bot_api.reply_message(
            event.reply_token,
            build_sos_form_flex("คุณ")
        )
    else:
        # Fallback to chat-based flow when LIFF not configured
        quick_reply = QuickReply(items=[
            QuickReplyButton(action=LocationAction(label="📍 ส่งพิกัดตำแหน่งแจ้งเหตุ"))
        ])
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(
                text="🚨 SOS แจ้งเหตุฉุกเฉิน\n\nโปรดกดปุ่มด้านล่างเพื่อส่งพิกัด:",
                quick_reply=quick_reply
            )
        )
...
def _start_needs_flow(event, user_id):
    """Start Needs workflow"""
    session = sessions.get(user_id)
    session.update(state="needs_location")
    update_legacy_state(user_id, "needs_location", {})
    ...
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(
            text="📌 แจ้งความต้องการสิ่งของ\n\nโปรดกดปุ่มด้านล่างเพื่อแชร์พิกัด:",
            quick_reply=quick_reply
        )
    )
...
def _start_registration(event, user_id):
    ...
    Prefers the Register LIFF form (richer UI, validated fields). Falls back
    to the old text-based step-by-step flow when REGISTER_LIFF_URL isn't set
    (e.g. local dev, or LIFF not created yet in the LINE console).
    """
    if REGISTER_LIFF_URL:
    ...
    session = sessions.get(user_id)
    session.update(state="register_first_name")
    update_legacy_state(user_id, "register_first_name", {})
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text="📝 ลงทะเบียน\n\nขั้นตอนที่ 1: โปรดพิมพ์ 'ชื่อ' ของคุณครับ")
    )
...
def _handle_ai_query(event, user_id, user_text, timestamp):
    ...
    ai_response = ask_gemini(
        "ตอบคำถามนี้ให้ครบถ้วน ชัดเจน เข้าใจง่าย ใช้ภาษาที่คนทั่วไปอ่านแล้วเข้าใจทันที "
...
def _process_weather(event, lat, lon, user_id, timestamp):
    """Process weather check"""
    session = sessions.get(user_id)
    show_loading_animation(user_id, loading_seconds=5)
    
    weather = get_live_weather_scraper(lat, lon)
