...
def get_supabase_client():
    ...
    try:
        ...
        # This specific error ("unexpected keyword argument 'proxy'") is a
        # well-known supabase-py/gotrue/httpx version-mismatch bug, NOT a
        # credentials problem. Fix: in requirements.txt, pin ONLY
        # `supabase==2.31.0` (or newer) and remove any separate pins of
        # gotrue/postgrest/httpx/realtime -- let supabase-py manage its own
        # sub-dependencies. Then do a clean reinstall (clear build cache on
        # your host) so old cached wheels aren't reused.
        print(
...
def test_supabase_connection():
    """
    Lightweight end-to-end connectivity test used by the /debug/supabase-test
    route. Tries to actually initialize the client AND run a trivial query,
    returning a structured diagnosis instead of just True/False.
    """
    result = {
        "env_vars_present": bool(SUPABASE_URL) and bool(SUPABASE_KEY),
        "client_initialized": False,
        "query_ok": False,
        "error": None,
    }
    if not result["env_vars_present"]:
        result["error"] = "SUPABASE_URL and/or SUPABASE_KEY are not set in the environment."
        return result
    ...
    try:
        ...
        result["error"] = f"create_client() failed: {type(e).__name__}: {e}"
        return result

    try:
        client.table("sync_metadata").select("id").limit(1).execute()
        result["query_ok"] = True
    except Exception as e:
        result["error"] = f"Client created but query failed: {type(e).__name__}: {e}"
