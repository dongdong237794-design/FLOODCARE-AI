...
def get_live_weather_scraper(lat: float, lon: float) -> str:
    """
    Optimized weather fetch with multi-layer caching
    """
    start = time.time()
    ...
    if not TMD_ACCESS_TOKEN or not requests:
        result = "⚠️ ไม่สามารถโหลดข้อมูลอากาศได้ (ไม่ได้ตั้งค่า TMD_ACCESS_TOKEN)\nกรุณาตรวจสอบสภาพอากาศจากแอปพยากรณ์อากาศโดยตรง"
    ...
    try:
        ...
        weather_map = {1: "แจ่มใส", 2: "เมฆบางส่วน", 3: "เมฆมาก", 4: "ครึ้ม",
                      5: "ฝนเล็กน้อย", 6: "ฝนปานกลาง", 7: "ฝนหนัก",
                      8: "ฝนฟ้าคะนอง", 9: "หนาวจัด", 10: "หนาว",
                      11: "เย็น", 12: "ร้อนจัด"}
        desc = weather_map.get(code, "ไม่ระบุ")
        
        result = f"🌡️ {temp} °C | 🌧️ {desc}\n💧 ชื้น {rh}% | 🍃 ลม {wind} m/s"
        cache.weather.set(cache_key, result)
        ...
        result = "⚠️ ไม่สามารถดึงข้อมูลอากาศได้ในขณะนี้\nกรุณาตรวจสอบจากแอปพยากรณ์อากาศโดยตรง"
        cache.weather.set(cache_key, result)
        return result
